...
class JRY_mapCtrl_Mortar: RscControlsGroupNoScrollbars
{
	...
	class controls
	{
		...
		class Ammo: RscCombo
		{
			onLBSelChanged = "_ctrl = _this # 0;
             _display = ctrlParent _ctrl;
              _colorTxt = _display displayCtrl 654321;
               _color = _display displayCtrl 104;
                if ((_this # 1) == 1) then
                {
                _colorTxt ctrlSetTextColor [1,1,1,1];
                {
                _color lbAdd _x;
                }
                forEach ['Blue','Green','Red','Purple','Yellow'];
                 _color lbSetCurSel 0;
                 _color ctrlEnable true;
                 }
                 else
                {
                _colorTxt ctrlSetTextColor [1,1,1,0.6];
                lbClear _color;
                 _color ctrlEnable false;
                };";
			idc = 103;
...
class JRYMap: RscDisplayEmpty
{
	class controlsBackground
	{
		class JRYMapControl: RscMapControl
		{
			...
			onMouseButtonDblClick = QUOTE((_this # 0) call FUNC(guidatapad););
		};
	};
	class controls{};
};

class CLASS(mapctrl_resup): RscControlsGroupNoScrollbars
{
	idc = 1112350;
	x = 0;
	y = 0;
	...
	class controls
	{
		...
		class CrateTxt: RscStructuredText
		{
			colorBackground[] = {0,0,0,0.7};
			// idc = 1100;
			text = "Crates ";
