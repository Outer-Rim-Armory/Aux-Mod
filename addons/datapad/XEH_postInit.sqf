...
["Reloaded",
{
	...
	if (_weapon isEqualTo QCLASS(Launcher_Datapad) && {(_unit ammo _weapon) > 0 && {_uiAction == false  }}) then
	{
...
["WeaponChanged",
{
	...
	if (_oldWeapon isEqualTo QCLASS(Launcher_Datapad) && {_uiAction == true }) then
	{
	...
	if (_newWeapon isEqualTo QCLASS(Launcher_Datapad) && {(_object ammo _newWeapon) > 0}) then
	{
