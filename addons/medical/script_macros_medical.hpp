// Originally created by the ACE 3 team, modified by DartRuffian
...
#define HEAD_DAMAGE_THRESHOLD EGVAR_ACE(medical,const_headDamageThreshold)
#define HEAD_DAMAGE_THRESHOLD_DEFAULT 1
#define ORGAN_DAMAGE_THRESHOLD EGVAR_ACE(medical,const_organDamageThreshold)
...
#define FATAL_SUM_DAMAGE_WEIBULL_K EGVAR_ACE(medical,const_fatalSumDamageWeibull_K)
#define FATAL_SUM_DAMAGE_WEIBULL_L EGVAR_ACE(medical,const_fatalSumDamageWeibull_L)
...
#define HEART_HIT_CHANCE EGVAR_ACE(medical,const_heartHitChance)
...
#define PENETRATION_THRESHOLD EGVAR_ACE(medical,const_penetrationThreshold)
#define PENETRATION_THRESHOLD_DEFAULT 0.35

// To be replaced by a proper blood pressure calculation
#define BLOOD_LOSS_KNOCK_OUT_THRESHOLD EGVAR_ACE(medical,const_bloodLossKnockOutThreshold)
...
// --- pain
#define PAIN_UNCONSCIOUS EGVAR_ACE(medical,const_painUnconscious)
#define PAIN_UNCONSCIOUS_DEFAULT 0.5

// Pain fade out time (time it takes until pain is guaranteed to be completly gone)
#define PAIN_FADE_TIME EGVAR_ACE(medical,const_painFadeTime)
...
#define LIMPING_DAMAGE_THRESHOLD EGVAR_ACE(medical,const_limpingDamageThreshold)
#define LIMPING_DAMAGE_THRESHOLD_DEFAULT 0.30

// Minimum limb damage required for fracture
#define FRACTURE_DAMAGE_THRESHOLD EGVAR_ACE(medical,const_fractureDamageThreshold)
...
#define MED_LOG_VARNAME(type) (format [QEGVAR_ACE(medical,log_%1), type])
...
#define VAR_BLOOD_PRESS       QEGVAR_ACE(medical,bloodPressure)
#define VAR_BLOOD_VOL         QEGVAR_ACE(medical,bloodVolume)
#define VAR_WOUND_BLEEDING    QEGVAR_ACE(medical,woundBleeding)
#define VAR_CRDC_ARRST        QEGVAR_ACE(medical,inCardiacArrest)
#define VAR_HEART_RATE        QEGVAR_ACE(medical,heartRate)
#define VAR_PAIN              QEGVAR_ACE(medical,pain)
#define VAR_PAIN_SUPP         QEGVAR_ACE(medical,painSuppress)
#define VAR_PERIPH_RES        QEGVAR_ACE(medical,peripheralResistance)
#define VAR_UNCON             "ACE_isUnconscious"
#define VAR_OPEN_WOUNDS       QEGVAR_ACE(medical,openWounds)
#define VAR_BANDAGED_WOUNDS   QEGVAR_ACE(medical,bandagedWounds)
#define VAR_STITCHED_WOUNDS   QEGVAR_ACE(medical,stitchedWounds)
// These variables track gradual adjustments (from medication, etc.)
#define VAR_MEDICATIONS       QEGVAR_ACE(medical,medications)
// These variables track the current state of status values above
#define VAR_HEMORRHAGE        QEGVAR_ACE(medical,hemorrhage)
#define VAR_IN_PAIN           QEGVAR_ACE(medical,inPain)
#define VAR_TOURNIQUET        QEGVAR_ACE(medical,tourniquets)
#define VAR_FRACTURES         QEGVAR_ACE(medical,fractures)

// - Unit Functions ---------------------------------------------------
// Retrieval macros for common unit values
// Defined for easy consistency and speed
#define GET_SM_STATE(_unit)         ([_unit, EGVAR_ACE(medical,STATE_MACHINE)] call CBA_statemachine_fnc_getCurrentState)
...
#define GET_DAMAGE_THRESHOLD(unit)  (unit getVariable [QEGVAR_ACE(medical,damageThreshold), [EGVAR_ACE(medical,AIDamageThreshold),EGVAR_ACE(medical,playerDamageThreshold)] select (isPlayer unit)])

// The following function calls are defined here just for consistency
#define GET_BLOOD_LOSS(unit)        ([unit] call EFUNC_ACE(medical_status,getBloodLoss))
#define GET_BLOOD_PRESSURE(unit)    ([unit] call EFUNC_ACE(medical_status,getBloodPressure))
